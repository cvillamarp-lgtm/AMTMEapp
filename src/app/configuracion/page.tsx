    ...
    <div className="grid gap-5 xl:grid-cols-[1fr_0.8fr]">
      <Card>
        <div className="flex items-center justify-between gap-3">
          ...
          <Badge tone={state.config.paletteLocked ? 'good' : 'warning'}>
            {state.config.paletteLocked ? 'Paleta bloqueada' : 'Paleta editable'}
          </Badge>
        </div>
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Bloqueo de paleta">
            <Input value={state.config.paletteLocked ? 'sí' : 'no'} readOnly />
          </Field>
          <Field label="Canales activos">
            <Input value={state.config.activeChannels.length.toString()} readOnly />
          </Field>
          <Field label="Formatos activos">
            <Input value={state.config.activeFormats.length.toString()} readOnly />
          </Field>
        </div>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          <Field label="IA primaria">
            <Input value={state.config.aiPrimaryProvider} readOnly />
          </Field>
          <Field label="IA fallback">
            <Input value={state.config.aiFallbackProvider} readOnly />
          </Field>
          <Field label="Modelo Grok">
            <Input value={state.config.aiPreferredModelByProvider.grok} readOnly />
          </Field>
          <Field label="Modelo Gemini">
            <Input value={state.config.aiPreferredModelByProvider.gemini} readOnly />
          </Field>
        </div>
        <div className="mt-5">
      ...
      <Card>
        ...
        <div className="mt-4 space-y-3 text-sm text-black/60">
          {state.config.futureApis.map((item) => (
            <div key={item} className="rounded-2xl border border-black/8 bg-[#F5F5F7] px-4 py-3">
              {item}
            </div>
          ))}
        </div>
        <div className="mt-5 rounded-3xl border border-black/8 bg-[#001F36] p-4 text-white">
          ...
          <p className="mt-3 text-sm leading-6 text-white/78">
            La interfaz debe sentirse limpia, precisa y premium. AMTME ocupa el centro, el ruido
            visual no tiene permiso.
          </p>
        </div>
        ...
        <div className="mt-4 rounded-3xl border border-black/8 bg-white p-4">
          ...
          <p className="mt-3 text-sm leading-6 text-black/58">
            La app ya puede enviar prompts a Grok o Gemini desde el servidor usando variables de
            entorno. Solo faltan XAI_API_KEY y GEMINI_API_KEY.
          </p>
        </div>
