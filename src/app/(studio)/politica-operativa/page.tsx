        ...
        {state.masterSections.map((section) => (
          <Card key={section.id} className="bg-white">
            <div className="flex items-center justify-between gap-2">
              <h3 className="text-lg font-semibold text-amtme-navy">{section.title}</h3>
      ...
      <Card>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {[
            ...
            <div
              key={rule}
              className="rounded-3xl border border-black/8 bg-amtme-cream p-4 text-sm leading-6 text-amtme-navy"
            >
              {rule}
            </div>
          ))}
