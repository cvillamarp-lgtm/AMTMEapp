...
export default function MetricasPage() {
  ...
  const [form, setForm] = useState({
    month: new Date().toISOString().slice(0,7), platform: '',
    reach: 0, plays: 0, downloads: 0, engagement: 0,
    profile_visits: 0, link_clicks: 0, dms: 0, conversions: 0,
    revenue: 0, insight: '', action: ''
  })
...
Devuelve SOLO el JSON. Español neutro. Sin clichés.`
    ...
    try {
      const res = await fetch('/api/ia/generar', {
        ...
        body: JSON.stringify({ provider: 'groq', prompt, systemPrompt: 'Eres el analista de métricas de AMTME. Respondes en JSON válido únicamente, sin markdown.' })
      })
          ...
          {loading ? <div className="text-center py-12 text-muted-foreground">Cargando...</div>
            : metrics.length === 0 ? (
              <Card><CardContent className="py-12 text-center">
                ...
                <Button className="bg-[#e8ff40] text-[#0c1f36] hover:bg-[#d4eb3a]" onClick={() => setDialogOpen(true)}><Plus className="mr-2 h-4 w-4" />Registrar primera métrica</Button>
              </CardContent></Card>
            ) : (
                    ...
                    <Card key={m.id}>
                      <CardHeader className="pb-3">
                        <div className="flex items-center justify-between">
                          ...
                          <Button size="sm" variant="outline" onClick={() => generateReport(m)} disabled={generating}>
                            <Sparkles className="h-4 w-4 mr-1" />{generating ? 'Generando...' : 'Generar reporte IA'}
                          </Button>
                        </div>
                      </CardHeader>
                      <CardContent>
                        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                          <div><p className="text-xs text-muted-foreground">Engagement</p><p className="text-2xl font-semibold">{kpis.engagementRate}%</p></div>
                          <div><p className="text-xs text-muted-foreground">Conv. Rate</p><p className="text-2xl font-semibold">{kpis.conversionRate}%</p></div>
                          <div><p className="text-xs text-muted-foreground">Plays→DM</p><p className="text-2xl font-semibold">{kpis.playsToDM}%</p></div>
                          <div><p className="text-xs text-muted-foreground">DMs</p><p className="text-2xl font-semibold">{m.dms}</p></div>
                          <div><p className="text-xs text-muted-foreground">Ingresos</p><p className="text-2xl font-semibold text-[#0c1f36]">${m.revenue.toFixed(0)}</p></div>
                        </div>
                        {m.insight && <p className="mt-3 text-sm border-t pt-3 text-muted-foreground"><span className="font-medium text-foreground">Insight: </span>{m.insight}</p>}
                      </CardContent>
                    </Card>
                  )
                })}
          ...
          <div className="space-y-4">
            {reports.length === 0 ? (
              <Card><CardContent className="py-12 text-center">
                ...
                <p className="text-xs text-muted-foreground mt-1">En la pestaña Métricas Mensuales, haz clic en "Generar reporte IA" en cualquier mes</p>
              </CardContent></Card>
            ) : reports.map(r => (
              <Card key={r.id}>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-muted-foreground">{new Date(r.generated_at).toLocaleDateString('es-MX')}</span>
                  </div>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <div className="rounded-xl bg-[#F5F2EA] p-4">
                      <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-1">Patrón de crecimiento</p>
                  ...
                  <div className="rounded-xl bg-[#e8ff40]/20 border border-[#e8ff40] p-4">
                    <p className="text-xs font-semibold uppercase tracking-wider text-[#0c1f36] mb-1">Recomendación 7 días</p>
                    <p className="text-sm font-medium">{r.recommendation_7d}</p>
                  </div>
                  <div className="rounded-xl bg-[#0c1f36] text-white p-4">
                    <p className="text-xs font-semibold uppercase tracking-wider text-white/60 mb-1">Hipótesis próximo episodio</p>
