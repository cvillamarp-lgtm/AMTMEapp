...
export const aiGenerateRequestSchema = z.object({
  ...
  prompt: z.preprocess(
    ...
    z
      .string()
      .min(1, 'Se requiere un prompt.')
      .max(
        MAX_AI_PROMPT_LENGTH,
        `El prompt excede el máximo de ${MAX_AI_PROMPT_LENGTH} caracteres.`
      )
  ),
  systemPrompt: z.preprocess(
    trimString,
    z.string().max(4000, 'El systemPrompt excede 4000 caracteres.').optional()
  ),
  model: z.preprocess(
    trimString,
    z
      ...
      .max(120, 'El modelo excede 120 caracteres.')
      .optional()
  ),
});
